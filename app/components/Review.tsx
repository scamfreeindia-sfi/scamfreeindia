...
export default function Review() {
    ...
    return (
        <div className="bg-[#050505] py-8 overflow-hidden border-t border-white/5 relative">
            <div className="max-w-7xl mx-auto px-4 relative z-10">
                ...
                <div className="relative h-[280px] w-full overflow-visible">
                    <div
                        ...
                        className="absolute left-1/2 flex items-center transition-transform duration-700 ease-in-out min-w-max"
                        style={{
                            gap: `${gap}px`,
                            transform: `translateX(calc(-1 * (${cardWidth / 2}px + ${activeIndex} * (${cardWidth + gap}px))))`
                        }}
                    >
                    ...
                    <div className="absolute bottom-[-40px] left-1/2 -translate-x-1/2 flex gap-2">
                        {reviews.map((_, i) => (
                            <button
                                ...
                                className={`w-2 h-2 rounded-full transition-all duration-300 ${i === activeIndex ? 'w-8 bg-[#EF4444]' : 'bg-white/10'}`}
                            />
                        ))}
                    </div>
                </div>
            ...
            <div className="absolute top-[30%] left-[-5%] text-[20vw] font-black text-white/[0.02] select-none pointer-events-none -rotate-12 uppercase italic whitespace-nowrap">
                Trusted
            </div>
            <div className="absolute bottom-[20%] right-[-5%] text-[20vw] font-black text-white/[0.02] select-none pointer-events-none rotate-12 uppercase italic whitespace-nowrap">
                Verified
            </div>
        </div>
    );
